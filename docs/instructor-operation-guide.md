...
- 정답과 해설은 각 퀴즈 항목에 포함되어 있으며, 단순 정답 확인이 아니라 "왜 그런지"까지 설명하도록
  학생에게 요구한다.
- 10차시는 별도 미니 프로젝트 산출물 없이, 9차시에서 만든 모델로 새 로트(`week09_new_lots_to_predict.csv`)를
  예측하는 실습 결과로 평가한다. 평가 시 다음을 확인한다.
  - `predict()`와 `predict_proba()` 결과를 모두 확인했는지(동일한 예측이라도 확신도가 다를 수
    있음을 이해했는지)
  - "정확도만으로 모델을 신뢰할 수 없다"(9차시 정확도의 함정)는 관점을 10차시 예측 해석에도
    이어서 적용했는지 여부
